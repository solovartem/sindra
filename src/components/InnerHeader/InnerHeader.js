...
import { Row, Col, Breadcrumb, Typography, Button, message, notification } from 'antd';
import { UploadOutlined } from '@ant-design/icons';
import { getEstCost, getEstCompletion } from '../../redux/selectors/estimation'
...
const InnerHeader = ({
  ...
  const handleSubmitInfo = async () => {
    ...
    if (isError.current) {
      return notification.error({message:"Incomplete form", description: "Please complete all required fields to submit your digital request"})
    } else {
      setLoading(true)
      submitInfo().then((res)=>{
        toggleFeedback(true);
  ...
  return (
    <>
      { showFeedback && <Feedback showFeedbackModal={showFeedback} feedbackEmail={feedbackEmail} /> }
      <Row style={{ backgroundColor: 'white', height: 'auto', padding: '10px 50px' }}>
        <Col span={24}>
          <Breadcrumb style={{ margin: '16px 0' }}>
            <Breadcrumb.Item>Home</Breadcrumb.Item>
            <Breadcrumb.Item>Forms</Breadcrumb.Item>
            <Breadcrumb.Item>New SoA</Breadcrumb.Item>
          </Breadcrumb>
        </Col>
        <Col span={24} style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
          ...
          <div>
            <Text>Est. cost: </Text>
            <Text className="fade-text initial-load" style={{ marginRight: '20px' }} strong>{localCost}</Text>
            <Text>Est. completion: </Text>
            <Text className="fade-text initial-load" strong>{localCompletion}</Text>
          </div>
        </Col>
        ...
        <Col span={24} style={{ display: 'flex', justifyContent: 'flex-end' }}>
          ...
          <div>
      <Button icon={<UploadOutlined />} type='primary' onClick={()=>handleSubmitInfo()} loading={loading}>Submit Form</Button>
          </div>
        </Col>
      </Row><br/>
    </>
